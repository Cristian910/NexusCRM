...
apiClient.interceptors.response.use(
  ...
  async (error: AxiosError) => {
    ...
    // 401 on any route except the auth endpoints themselves (login/register/refresh
    // don't have a session yet, so there's no token to refresh — a 401 there is a
    // real auth failure, not an expired-session case).
